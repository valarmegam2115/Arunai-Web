...
const matchRoute = (pathname) => {
  ...
  // prefix match (for nested routes not yet in table → show null so we can add 404 later)
  return null
}
  ...
  return (
    <div className="min-h-screen bg-white text-slate-900">
      ...
      {matched ? (
        ...
        // Fallback – plain slate background for unbuilt pages
        <div className="flex min-h-screen items-center justify-center bg-slate-50 pt-28">
          <div className="text-center">
            <p className="text-6xl font-black text-[#001a66]">404</p>
            <p className="mt-3 text-xl font-semibold text-gray-500">Page not built yet</p>
            <a
              href="/"
              onClick={e => { e.preventDefault(); window.history.pushState({}, '', '/'); window.dispatchEvent(new PopStateEvent('popstate')) }}
              className="mt-6 inline-block rounded-lg bg-[#001a66] px-8 py-3 text-sm font-bold text-white hover:bg-[#0b2a8a]"
            >
              ← Back to Home
            </a>
          </div>
